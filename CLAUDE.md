...
CatPuzzle is a "one cat per row/column/color-group, no adjacent cats" logic puzzle. The codebase is split into two strictly separated layers:
...
- `Cell` / `CellState` — board coordinate + color group, and the three-state cell value (empty/excluded/cat).
...
- `LevelDefinition` — id, size, cat count, max mistakes, color-ID grid. `BuiltInLevels` ships three verified 6×6 `LevelFixture`s (each paired with a known-correct solution). `LevelValidator` checks a definition's internal consistency (dimensions, cat/color counts, mistake config) without imposing color connectivity.
...
- Two independent solvers exist for different purposes:
  ...
  - `LogicalPuzzleSolver` — models per-cell candidates and emits deterministic, explainable step-by-step deductions (row/column/color/confirmed-cat propagation). Supports `.logicOnly` (used for the shipped levels) and bounded `.challenge(maxAssumptionDepth:)` proof-by-contradiction. Any report that required an assumption is classified as `challenge`.
